# -*- coding: utf-8 -*-
"""
purrmaster_wet_crawl.py
爬取 https://purrmaster.com/brand/completewet 全部貓主食罐
- 品牌列表 / 品牌頁：requests（靜態）
- 產品詳情頁：Playwright（JS 渲染）
輸出到 data/purrmaster_wet_database.xlsx + data/raw/purrmaster_wet_YYYYMMDD_HHMM.json
⚠️  不寫入 Supabase，請老闆確認後手動觸發上傳
"""
...
LIST_URL = f"{BASE}/brand/completewet"
DELAY    = 1.5       # 秒，靜態頁面
...
OUT_EXCEL  = os.path.join(SCRIPT_DIR, "..", "data", "purrmaster_wet_database.xlsx")

# 主食罐水分通常 70-85%，乾物比才是重點
# has_grain 必須用成分原料判斷，不能用品名
...
COL_WIDTHS = {
    "品牌": 14, "飼料名稱": 35, "食品類型": 8, "適用階段": 8, "產地": 8,
...
def get_brand_links() -> list[tuple[str, str]]:
    ...
    if not soup:
        return []
    brands, seen = [], set()
    # 主食罐品牌連結：/brand/completewet/[uuid]
    for a in soup.find_all("a", href=re.compile(r"/brand/completewet/[0-9a-f-]{36}")):
        href = a.get("href", "")
...
def get_product_links(brand_url: str) -> list[str]:
    ...
    if not soup:
        return []
    links, seen = [], set()
    # 主食罐產品連結：/product/completewet/[uuid]
    for a in soup.find_all("a", href=re.compile(r"/product/completewet/[0-9a-f-]{36}")):
        href = a.get("href", "")
        url  = BASE + href if href.startswith("/") else href
        if url not in seen:
            seen.add(url)
...
def scrape_product(page, brand_name: str, url: str) -> dict | None:
    ...
    # 成分原料（has_grain 必須根據成分表判斷，不能看品名）
    ing_m = re.search(
...
def main():
    print("=" * 55)
    print("  Purrmaster 主食罐爬蟲（completewet）")
    print("  ⚠️  輸出 JSON/Excel 只，不寫入 Supabase")
    print("=" * 55)
    ...
    if all_foods:
        ...
        raw_path = os.path.join(raw_dir, f"purrmaster_wet_{time.strftime('%Y%m%d_%H%M')}.json")
        with open(raw_path, "w", encoding="utf-8") as f:
            json.dump(all_foods, f, ensure_ascii=False, indent=2)
        print(f"JSON 備份：{raw_path}")
        save_excel(all_foods)

    print(f"\n完成！{len(all_foods)} 筆資料等待老闆確認後再上傳 Supabase")
