...
const SCORE_LEVELS = [
  { range: '70–100', label: '優質主食', bg: '#e8f9ee', color: '#1a7f37', desc: '蛋白質充足、品質認證完整，適合作為長期日常主食。' },
  { range: '55–69',  label: '均衡日常', bg: '#e6f0fb', color: '#1554a0', desc: '整體品質良好，日常餵食無虞，偶爾輪替更好。' },
  { range: '40–54',  label: '基礎配方', bg: '#fff3e0', color: '#b35c00', desc: '基本營養需求可達，建議搭配其他優質飼料輪替。' },
  { range: '40 以下', label: '建議搭配', bg: '#ffeaea', color: '#c0392b', desc: '不建議長期單獨餵食，需搭配其他營養來源。' },
]

const PROTEIN_TIERS = [
  { range: '≥ 50%',   pts: '40', color: '#1a7f37' },
  { range: '45–49%',  pts: '37', color: '#3a7d3c' },
  { range: '40–44%',  pts: '33', color: '#6b9e6f' },
  { range: '35–39%',  pts: '29', color: '#b35c00' },
  { range: '30–34%',  pts: '26', color: '#c0702b' },
  { range: '26–29%',  pts: '15', color: '#d04020' },
  { range: '< 26%',   pts: '5',  color: '#c0392b' },
]

const CARB_TIERS = [
  { range: '≤ 10%',   pts: '25', color: '#1a7f37' },
  { range: '11–20%',  pts: '22', color: '#6b9e6f' },
  { range: '21–30%',  pts: '18', color: '#b35c00' },
  { range: '31–40%',  pts: '16', color: '#c0702b' },
  { range: '41–50%',  pts: '14', color: '#d04020' },
  { range: '> 50%',   pts: '7',  color: '#c0392b' },
]
  ...
  return (
    <main className="min-h-screen" style={{ background: '#f5f5f7' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-16">
        ...
        <div className="rounded-2xl overflow-hidden mb-8" style={{ background: '#fff', border: '0.5px solid #e5e7eb' }}>
          <div className="px-5 py-4" style={{ borderBottom: '0.5px solid #f3f4f6' }}>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold" style={{ color: ACCENT }}>最高 35 分</span>
            </div>
            <p className="text-xs text-gray-400 mt-1">品牌的研發投入、資訊透明度，反映飼料的整體可信度。</p>
          </div>
          <div className="px-5 py-3">
            {[
              { pts: '18', label: 'AAFCO 或 FEDIAF 認證', desc: '通過美國（AAFCO）或歐盟（FEDIAF）飼料管理協會認證，代表配方符合貓咪完整營養需求最低標準。皇家、希爾斯均持有此認證。' },
              { pts: '12', label: '公開完整成分列表',       desc: '品牌主動公開每款飼料的原料成分，讓消費者知道餵了什麼。' },
        ...
        <div className="rounded-2xl overflow-hidden mb-8" style={{ background: '#fff', border: '0.5px solid #e5e7eb' }}>
          {[
            { brand: '希爾斯（典型配方）', p: '35%', c: '38%', cert: 'AAFCO ✓', ingr: '✓', ash: '—', sp: 29, sc: 16, sq: 30, total: 75, label: '優質主食', color: '#1a7f37' },
            { brand: '皇家（E42 泌尿道）', p: '45.6%', c: '24.7%', cert: 'AAFCO ✓', ingr: '✓', ash: '✓', sp: 37, sc: 22, sq: 35, total: 94, label: '優質主食', color: '#1a7f37' },
            { brand: '高蛋白無穀品牌（示例）', p: '48%', c: '12%', cert: 'AAFCO ✓', ingr: '✓', ash: '✓', sp: 37, sc: 25, sq: 35, total: 97, label: '優質主食', color: '#1a7f37' },
            { brand: '廉價無認證品牌（示例）', p: '28%', c: '44%', cert: '—', ingr: '✓', ash: '—', sp: 15, sc: 14, sq: 12, total: 41, label: '基礎配方', color: '#b35c00' },
          ].map((ex, i) => (
            <div key={i} className="px-5 py-4" style={{ borderTop: i > 0 ? '0.5px solid #f3f4f6' : 'none' }}>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                ...
                <span>品質 {ex.cert}／成分{ex.ingr}／灰分{ex.ash} → <strong>{ex.sq}</strong>分</span>
              </div>
