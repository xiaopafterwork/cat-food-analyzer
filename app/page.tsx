    ...
    <main className="min-h-screen" style={{ background: '#f5f5f7' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-24">
        ...
        <div className="text-center pt-10 pb-8">
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: ACCENT, letterSpacing: '0.08em' }}>
            台灣最完整貓飼料評鑑
          </p>
        ...
        <div className="flex gap-2 flex-wrap mb-8">
          {([
            ...
            const active = item.value === null
              ? lifeStageFilter === null && !grainFilter
              : lifeStageFilter === item.value
            return (
              <button
                key={item.label}
                onClick={() => {
                  setLifeStageFilter(item.value)
                  if (item.value === null) setGrainFilter(false)
                }}
        ...
        {/* List */}
        <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3" style={{ letterSpacing: '0.08em' }}>
          評分排行
        </p>
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-400 py-16">找不到符合的飼料</p>
        ) : (
                ...
                <div key={food.id} className="flex items-center gap-2">
                  ...
                  <Link href={`/food/${food.id}`} className="flex-1 min-w-0">
                    ...
                    >
                      {/* Score circle */}
                      <div
                        className="w-14 h-14 rounded-full flex items-center justify-center shrink-0 font-bold text-xl"
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 truncate mb-0.5" style={{ fontSize: 15 }}>
                          {food.name}
                        </p>
                      ...
                      <svg className="shrink-0 text-gray-300" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      {compareIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 rounded-full shadow-xl"
